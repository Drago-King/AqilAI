...
private void run(String text) { if (status != null) status.setText("Thinking..."); AqilAccessibilityService.runCommand(text); AgentBrain.ask(this, text, a -> runOnUiThread(() -> { if (status != null) status.setText(a); if (AgentBrain.getBool(this, "tts_enabled", false)) VoiceEngine.speak(this, a, msg -> runOnUiThread(() -> { if (status != null) status.setText(msg + "\n" + a); })); })); }  
